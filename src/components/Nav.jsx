    ...
    <nav className="w-full fixed top-0 left-0 z-50">
      <div className={` ${isDark ? 'bg-black text-white drop-shadow-md flex items-center justify-between p-4 md:p-5': 'drop-shadow-md flex items-center justify-between p-4 md:p-5 bg-white'} `}>
        ...
        <ul  onClick={()=>setOpen(false)} className={` pl-9 md:flex transition-all ease-in duration-500 md:items-center md:pb-0 pb-12 md:w-auto md:pl-0 md:z-auto z-[-1] left-0 w-full  absolute md:static ${open?'top-5 text-white opacity-100 bg-black h-screen  flex flex-col justify-start mt-10 items-start':' top-[-490px] opacity-0 '} md:opacity-100 `}>
            ...
            <span className='md:hidden flex  text-3xl'>{auth.image ? <img className=' h-8 w-8 rounded-full overflow-hidden' src={auth.image} alt="" /> : <RxAvatar />}</span>
            <span className=' md:hidden flex' >{auth._id ? <p className=' cursor-pointer' onClick={() => { dispatch(logOutUser()); toast.success("logged out successfully") }} >Logout</p>  :<Link to={"/login"}>Login</Link>}</span>
           </li>  
          {Links.map((link) => (
              <li key={link.name} className=' md:ml-8 text-xl md:my-0 my-6'>
                <Link to={link.link} className={` hover:text-blue-500 duration-500 ${pathMatchRoute(link.link) && ' text-blue-500 border-b-2 border-b-blue-500'}`}>{link.name}</Link>     
              </li>
  
            ))}
          <li className=''>
            {auth.email===import.meta.env.VITE_ADMIN_EMAIL && <Link to={'/uploadproduct'}>< RiAdminFill className=' text-3xl md:ml-6 my-5 md:my-0 '/></Link>}
          </li>
        </ul>
        <div className=' gap-6 flex items-center justify-center'>

          <div className=' hidden text-3xl md:flex cursor-pointer' onClick={()=>setIsDark(!isDark)}>
          ...
          <div onClick={()=>setShowLogin(prevstate=>!prevstate)} className='relative'>
            <button className=' hidden md:flex text-3xl'>{auth.image ? <img className=' h-8 w-8 rounded-full overflow-hidden' src={auth.image} alt="" />: <RxAvatar />}</button>
            {showLogin && <div className='hidden md:flex text-black p-1 absolute border -left-3 top-8 bg-white text-sm cursor-pointer whitespace-nowrap'>{auth._id ? <p onClick={() => { dispatch(logOutUser()); toast.success("logged out successfully"); navigate("/") }}>Log Out</p>:<Link to={"/login"}>Log in</Link>}</div>}
            </div>
        </div>
               </div>
    </nav>
